...
class FaultListGenerator:
    ...
    def get_weight_fault_list(self,
        ...
        fault_list_filename = f'{cwd}/output/fault_list/{self.network_name}/{seed}_parameters_fault_list.csv'

        try:
            if load_fault_list:
                print('Loading fault list from file ...')
                with open(fault_list_filename, newline='') as f_list:
                    reader = csv.reader(f_list)
                ...
                print(f'Fault list {fault_list_filename} loaded from file')
            ...
            print(f'Fault list not found!')
            print(f'{fault_list_filename}')
            exit(1)
            '''
            ...
            # The population of faults
            total_possible_faults = np.sum(possible_faults_per_layer)
            print(f'total possible faults: {total_possible_faults}')
            # The percentage of fault to inject in each layer
            ...
            # Compute the number of fault to inject in each layer
            injected_faults_per_layer = [math.ceil(probability * n) for probability in probability_per_layer]
            print(f'total faults to inject: {round(n)}')
            print(f'injected faults per layer: {injected_faults_per_layer}')
