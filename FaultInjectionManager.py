...
import copy
from utils import clean_inference
import numpy as np
import torch
...
class FaultInjectionManager:
    ...
    def __init__(self,
        ...
        self.__faulty_output_folder = faulty_output_folder
    ...
    def run_faulty_campaign_on_weight(self,
        ...
        with torch.no_grad():
            ...
            for batch_id, batch in enumerate(self.loader):
                ...
                for fault_id, fault in enumerate(pbar):
                    ...
                    # clean_scores, clean_indices, diff_pred = self.__run_inference_on_batch(batch_id=batch_id,
                                                                                                    # data=data)
                    ###############################################
                    ...
                    # print(f'network: ResNet20')
                    # dataset_size = dataset_size + len(target)
                    ...
                    # if clean_indices == faulty_indices:
                    #     print(f'clean indices: {clean_indices}')
                    #     print(f'length: {len(clean_indices)}')
                    #     counter = counter + 1
                ...
                os.makedirs(f'{self.__log_folder}', exist_ok=True)
                log_filename = f'{self.__log_folder}/batch_{batch_id}.csv'
                ...
                if save_output:
                    os.makedirs(f'{self.__faulty_output_folder}', exist_ok=True)
                    np.save(f'{self.__faulty_output_folder}/batch_{batch_id}', self.faulty_output)
                    self.faulty_output = list()
        ...
        # Final log
        os.makedirs(f'{self.__log_folder}', exist_ok=True)
        log_filename = f'{self.__log_folder}/all_batches.csv'
        with open(log_filename, 'w') as log_file:
            log_writer = csv.writer(log_file)
